...
<c:set value="商家审核"  var ="pageTitle"></c:set>
...
<div class="myBodyDiv" >
  <form action="${ctx }${baseMapper}comparePirceList" method="post" id="searchForm">
    ...
    <table class="table table-striped table-bordered">
    	...
    	<tr>
    		<td><a href="${ctx }${baseMapper }verifyShop?id=${shop.id}">${shop.name }</a></td>
    		<td><fmt:formatDate value="${shop.createTime }" pattern="yyyy-MM-dd"/></td>
    		<td>${shop.telNum }</td>
    		<td><a href="${ctx }${baseMapper }verifyShop?id=${shop.id}">审核</a></td>
    	</tr>
    	</c:forEach>
    </table>
